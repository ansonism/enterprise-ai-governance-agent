...
from ai_governance.config import load_config
...
def test_environment_override_is_typed() -> None:
    settings = load_config(
        DEFAULT_CONFIG,
        environment={
            "AI_GOVERNANCE__PROVIDER__MAX_RETRIES": "5",
            "AI_GOVERNANCE__TOOLS__ALLOW_MUTATIONS": "false",
        },
    )
...
def test_unsafe_approval_configuration_is_rejected() -> None:
    with pytest.raises(ValidationError, match="Mutations cannot be enabled"):
        load_config(
            DEFAULT_CONFIG,
            environment={
                "AI_GOVERNANCE__TOOLS__ALLOW_MUTATIONS": "true",
                "AI_GOVERNANCE__APPROVAL__MODE": "disabled",
            },
        )
