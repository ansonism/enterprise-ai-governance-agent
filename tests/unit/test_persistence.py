...
from ai_governance.models import GovernanceAssessmentInput, RunState
from ai_governance.persistence import JsonRunStateRepository


def state() -> RunState:
    return RunState(
        run_id="safe-run_01",
        request=GovernanceAssessmentInput(case_id="case", title="Title", description="Description"),
    )
...
def test_json_repository_retries_transient_replace_lock(tmp_path: Path) -> None:
    ...
    with patch("ai_governance.persistence.os.replace", side_effect=flaky_replace):
